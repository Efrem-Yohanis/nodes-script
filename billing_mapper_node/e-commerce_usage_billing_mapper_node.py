...
logger = logging.getLogger("ecommerce_billing_cdr")
...
def extract_bucket_names(subs: List[dict], limit: int = 5) -> List[str]:
    out = []
    for s in subs:
...
def extract_postpaid_usages(subs: List[dict]) -> List[str]:
    outs = []
    for s in subs:
        for charge in (s.get('recordSubExtensions', []) or []):
            if charge.get('recordProperty') != 'chargingServiceInfo':
...
# gross/tax extraction

def find_first_account_for_gross(subs: List[dict]) -> Optional[dict]:
    # prefer accountInfo values present, otherwise first occurrence
    for s in subs:
        for charge in (s.get('recordSubExtensions', []) or []):
            if charge.get('recordProperty') != 'chargingServiceInfo':
                continue
...
def map_ecommerce(cdr_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out['EL_CALL_VOLUME'] = ''
    out['EL_BAND_LABEL_AMA_CODE'] = ''

    # 9 applied discount ids
    applied = extract_bucket_names(subs, limit=5)
    out['EL_APPLIED_DISCOUNT_ID'] = '|'.join(applied)

    # 10 duplicate event label
    out['EL_EVENT_LABEL_2'] = out['EL_EVENT_LABEL']

    out['EL_ORIGINATING_ZONE_CODE'] = ''

    # 12 processed timestamp generationTimestamp
    out['EL_PROCESSED_TIMESTAMP'] = parse_ts(elems.get('generationTimestamp') or '')

    # 13 plan id: OfferCode or bundleName list
    offer_code = elems.get('OfferCode') or elems.get('offerCode') or ''
    plans = []
    for s in subs:
    ...
    # 16 event result mapping includes '00'
    ...
    # include string '00' or numeric 0
    if str(res_code) == '00' or rc_int == 0:
        out['EL_EVENT_RESULT'] = 1
    elif rc_int in success_codes:
        out['EL_EVENT_RESULT'] = 1
    else:
    ...
    # process filename - try metadata or $sourceFileName
    procfn = safe_get(cdr_json, ['metadata','filename']) or safe_get(generic, ['recordElements','sourceFileName']) or cdr_json.get('$sourceFileName') or ''
    out['EL_PROCESS_FILENAME'] = procfn

    out['EL_CUG_ENABLED'] = 'false'
    out['EL_APPLIED_FAMILY_GROUP_DISCOUNT_IDS'] = ''

    out['EL_POSTPAIDBUCKETID'] = ','.join(plans)
    out['EL_POSTPAIDBUCKETUSAGES'] = '|'.join(extract_postpaid_usages(subs))

    # 23 CDR reference number from recordId
    out['EL_CDR_REFERENCE_NUMBER'] = safe_get(elems, ['recordId']) or ''

    # 24 hard coded 0
    out['EL_ROUNDED_CALL_DURATION'] = 0

    out['EL_ROUNDED_CALL_VOLUME'] = ''

    out['EL_CHARGE_CODE'] = elems.get('TransactionId') or elems.get('transactionId') or elems.get('TransactionID') or ''
    out['EL_PLAN_NAME'] = elems.get('OfferName') or elems.get('offerName') or ''
...
def process_input_file(path: Path, out_dir: Path, processed_dir: Path) -> None:
    ...
    out_path = out_dir / f"{path.stem}_ecom_billing_phase1.json"
