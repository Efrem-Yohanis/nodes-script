...
logger = logging.getLogger("sms_billing_cdr")
...
def extract_subscription_id_types(rec: dict) -> (str,str):
    msisdn = ''
    imsi = ''
    for ext in rec.get('recordExtensions', []) or []:
...
def band_label_for_sms(a: str, b: str) -> str:
    if not a or not b:
        return 'onnet'
    a_s = str(a)
    b_s = str(b)
    if a_s.startswith('2517') and len(a_s) > 10 and b_s.startswith('2517') and len(b_s) > 10:
        return 'onnet'
...
def map_sms(cdr_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1 EL_ACCOUNT_ID
    msisdn, imsi = extract_subscription_id_types(generic)
    out['EL_ACCOUNT_ID'] = msisdn or ''

    # 2 EL_DIALLED_DIGITS recipientAddress
    ...
    # 8 Band label using originatorAddress (A) and recipientAddress (B)
    ...
    out['EL_BAND_LABEL_AMA_CODE'] = band_label_for_sms(a, b)

    # 9 applied discount (bucket names up to 5)
    applied = []
    for s in subs:
        for charge in (s.get('recordSubExtensions', []) or []):
            if charge.get('recordProperty') != 'chargingServiceInfo':
...
def process_input_file(path: Path, out_dir: Path, processed_dir: Path) -> None:
    ...
    out_path = out_dir / f"{path.stem}_sms_billing_phase1.json"
